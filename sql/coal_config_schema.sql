...
create index if not exists idx_cfg_device_section on coal_config.cfg_device(process_section);

insert into coal_config.cfg_device(device_code, device_name, device_type, process_section, protocol_type)
values
('DEV-BELT-001', '原煤皮带机1', 'BELT', 'RAW_COAL', 'MODBUS'),
('DEV-PUMP-001', '介质泵1', 'PUMP', 'HEAVY_MEDIA', 'OPC_UA')
on conflict (device_code) do nothing;

insert into coal_config.cfg_point(point_code, device_id, point_name, data_type, unit, rw_type, addr, collect_cycle_ms, alarm_enable)
select 'PT-BELT-001-SPEED', d.id, '皮带速度', 'DOUBLE', 'm/s', 'R', '40001', 500, true
from coal_config.cfg_device d
where d.device_code = 'DEV-BELT-001'
on conflict (point_code) do nothing;

create or replace view coal_config.v_device_point as
select d.id as device_id, d.device_code, d.device_name, p.id as point_id, p.point_code, p.point_name
from coal_config.cfg_device d
left join coal_config.cfg_point p on d.id = p.device_id;
